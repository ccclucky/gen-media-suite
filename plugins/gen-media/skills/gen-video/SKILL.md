...
> 语言铁律：所有面向用户的文字一律中文——brief 卡片的表头与内容、确认门、提问、报错解释，全中文；送给模型的 prompt 也用中文（这些模型中文表现正常）。脚本 stdout 是英文，读完后翻译再回。
...
- 不加 logo、字幕、人物，除非用户要。
- 镜头词汇、时间线模板、失败对策 → `references/prompt-guide.md`。
- 完成判定：六槽位 + 格式填好，**中文卡片摆出**（模板见确认门）。本步不生成。
...
- 例外：用户基于上一条视频的迭代指令（「换俯冲镜头」「再来一版更长」）本身就是确认，直接生成，不必重摆整张卡。
